...
/*
 * The "array-error" runtime functions
 */
int array_tensor_error_null (int64_t instructionID){

  /*
   * Check the inputs.
   */
  if ((instructionID & 0x1) != 0x1){
    printf("ERROR: the first input of a call to \"tensor-error (line)\" is not encoded\n");
    abort();
  }
...
int array_error (int64_t instructionID, int64_t length, int64_t index) {

  /*
   * Check the inputs.
   */
  if ((instructionID & 0x1) != 0x1){
    printf("ERROR: the first input of a call to \"tensor-error (line, length, index)\" is not encoded\n");
    abort();
  }
  if ((length & 0x1) != 0x1){
    printf("ERROR: the second input of a call to \"tensor-error (line, length, index)\" is not encoded\n");
    abort();
  }
  if ((index & 0x1) != 0x1){
    printf("ERROR: the third input of a call to \"tensor-error (line, length, index)\" is not encoded\n");
    abort();
  }
   ...
   */
  if (index < length){
    printf("ERROR: a call to \"tensor-error (line, length, index)\" is incorrect, index is within the accepted range\n");
    abort();
  }
...
int tensor_error (int64_t instructionID, int64_t dimension, int64_t length, int64_t index) {

  /*
   * Check the inputs.
   */
  if ((instructionID & 0x1) != 0x1){
    printf("ERROR: the first input of a call to \"tensor-error (line, dimension, length, index)\" is not encoded\n");
    abort();
  }
  if ((dimension & 0x1) != 0x1){
    printf("ERROR: the second input of a call to \"tensor-error (line, dimension, length, index)\" is not encoded\n");
    abort();
  }
  if ((length & 0x1) != 0x1){
    printf("ERROR: the third input of a call to \"tensor-error (line, dimension, length, index)\" is not encoded\n");
    abort();
  }
  if ((index & 0x1) != 0x1){
    printf("ERROR: the forth input of a call to \"tensor-error (line, dimension, length, index)\" is not encoded\n");
    abort();
  }
   ...
   */
  if (index < length){
    printf("ERROR: a call to \"tensor-error (line, dimension, length, index)\" is incorrect, index is within the accepted range\n");
    abort();
  }
